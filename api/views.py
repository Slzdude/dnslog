...
from logview.models import DnsLog, WebLog


def weblog(request, apikey, hashstr):
    user = ApiKey.objects.filter(key=apikey)
    if user:
        weblogs = WebLog.objects.filter(user=user[0].user).filter(
            path__contains=hashstr
        )
        data = []
        ret = {}
        if weblogs:
...
def dnslog(request, apikey, hashstr):
    user = ApiKey.objects.filter(key=apikey)
    if user:
        dnslogs = DnsLog.objects.filter(user=user[0].user).filter(
            host__contains=hashstr
        )
        data = []
        if dnslogs:
