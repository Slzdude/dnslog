...
from logview.models import DnsLog, UserSubDomain, WebLog
...
def index(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "logview/login.html", {"error": "username or password error!"}
        )
    else:
...
def selfinfo(request):
    ...
    vardict = {"type": "info", "subdomain": subdomain, "apikey": apikey}
...
def manage_user_add(request):
    ...
    if user.is_superuser:
        ...
        if subdomain == "":
            ...
            check_subdomain = UserSubDomain.objects.filter(subdomain=random_str)
            while check_subdomain:
                random_str = "".join(
                    random.sample(string.ascii_letters + string.digits, 5)
                )
            subdomain = random_str
        else:
            check_subdomain = UserSubDomain.objects.filter(subdomain=subdomain)
            if check_subdomain:
                ret = {"status": -1, "msg": "subdomain already exist!"}
                return JsonResponse(ret)

        # information exist check
        ...
        try:
            with transaction.atomic():
                ...
                usersubdomain = UserSubDomain(
                    user=adduser, subdomain=subdomain, status=1
                )
